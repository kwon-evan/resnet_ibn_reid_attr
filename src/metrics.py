...
class ReIDMetrics:
    ...
    def __init__(self, num_classes: int, device: str = "cuda"):
        ...
        self.id_accuracy = Accuracy(task="multiclass", num_classes=num_classes).to(device)
        
    def compute_cmc_map(self, embeddings: torch.Tensor, person_ids: torch.Tensor, 
                       gallery_embeddings: torch.Tensor, gallery_ids: torch.Tensor,
                       k_values: list = [1, 5, 10, 20], batch_size: int = 100):
        """
